...
namespace P1test.Items.Weapons
{
	public class MiniShot : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			...
			{
				Vector2 perturbedSpeed3 = velocity.RotatedByRandom(MathHelper.ToRadians(16)); // 30 degree spread.
				// If you want to randomize the speed to stagger the projectiles
				float scale = 1f - (Main.rand.NextFloat() * .3f);
				perturbedSpeed3 = perturbedSpeed3 * scale;
				Vector2 perturbedSpeed2 = velocity.RotatedByRandom(MathHelper.ToRadians(16)); // 30 degree spread.
				                                                                      // If you want to randomize the speed to stagger the projectiles
...

			{
				...
				Vector2 dustOffset = Vector2.Normalize(velocity) * 64f;//32
