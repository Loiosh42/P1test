...
namespace P1test.Items.Utility
{
	public class SkyDrill : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.shoot = 143; //idk why but all the guns in the vanilla source have this
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
			...
			for (int index = 0; index < numberProjectiles; ++index)
			{
				...
				float SpeedX = 0f;
				float SpeedY = 10f;
				Projectile.NewProjectile(source, numx, numy, SpeedX, SpeedY, type, damage, knockback, Main.myPlayer, 0.0f, (float)Main.rand.Next(5));
			}
