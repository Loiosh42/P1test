			...
			if (Main.myPlayer == Projectile.owner)
			{
				...
				if (Projectile.ai[1] > (float)Main.rand.Next(35, 200))//(5 + ((int)Main.MouseWorld.Length() / 640 ), 20 + ((int)Main.MouseWorld.Length() / 640)))
				{
					...
					for (int num162 = 0; num162 < num161; num162++)
					{
						...
						Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, vector12.X, vector12.Y, Projectile.type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, -1000f);
